#!/usr/bin/env python3
"""CDLO drift dashboard — render ledger_log.jsonl as a self-contained HTML page.

Usage:
    python scripts/build_drift_dashboard.py [--log ledger_log.jsonl] [--out drift_dashboard.html]
...
COLORS = {
    "AUTHORIZE EXECUTION": "#3fb950",
    "MANDATORY LIFECYCLE PIVOT": "#d29922",
    "EXECUTION FREEZE": "#58a6ff",
    "INTEGRATION HALT": "#f85149",
}
...
def thresholds():
    ...
    if os.path.exists(cfg_path):
        ...
        m = re.search(r"pivot_threshold\s*\|\s*([0-9.]+)", txt)
        if m: pivot = float(m.group(1))
        m = re.search(r"halt_threshold\s*\|\s*([0-9.]+)", txt)
...
def main():
    ...
    ap.add_argument("--log", default="ledger_log.jsonl")
    ap.add_argument("--out", default="drift_dashboard.html")
    args = ap.parse_args()
    ...
    for i, e in enumerate(entries):
        ...
        y = h - pad - (h - 2 * pad) * min(float(e.get("cds", 0)), 1.0)
        pts.append(f"{x:.0f},{y:.0f}")
        c = COLORS.get(e.get("directive", ""), "#8b949e")
        dots.append(f'<circle cx="{x:.0f}" cy="{y:.0f}" r="5" fill="{c}"><title>{html.escape(str(e.get("target","")))} — CDS {e.get("cds")}</title></circle>')
...
<text x="{w-pad+2}" y="{ty_p:.0f}" fill="#d29922" font-size="11" dominant-baseline="middle">PIVOT {pivot}</text>
<line x1="{pad}" y1="{ty_h:.0f}" x2="{w-pad}" y2="{ty_h:.0f}" stroke="#f85149" stroke-dasharray="6 4"/>
<text x="{w-pad+2}" y="{ty_h:.0f}" fill="#f85149" font-size="11" dominant-baseline="middle">HALT {halt}</text>
{polyline}{''.join(dots)}
</svg>"""
    ...
    for e in reversed(entries):
        ...
        rows += (f'<tr><td>{html.escape(str(e.get("ts","")))}</td>'
                 f'<td>{html.escape(str(e.get("target","")))}</td>'
                 f'<td>{e.get("cci","")}</td><td>{e.get("cds","")}</td>'
                 f'<td style="color:{c};font-weight:600">{html.escape(d)}</td>'
                 f'<td style="color:#8b949e">{html.escape(str(e.get("routing","")))}</td></tr>')
    if not entries:
        rows = '<tr><td colspan="6" style="color:#8b949e">No verdicts logged yet — run the operator in Claude Code and ledger_log.jsonl will populate.</td></tr>'

    page = f"""<!doctype html><html><head><meta charset="utf-8"><title>CDLO drift dashboard</title>
...
<p style="color:#8b949e">Context-Drift Score per verdict against the gate thresholds. The story to watch is the trend: scores climbing toward the halt line mean the work is decoupling from declared intent even while individual changes still pass.</p>
{chart}
<table><tr><th>Timestamp</th><th>Target</th><th>CCI</th><th>CDS</th><th>Directive</th><th>Routing</th></tr>{rows}</table>
</body></html>"""
